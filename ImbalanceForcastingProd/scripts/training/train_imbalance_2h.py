...
def load_weather():
    """Load Bardejov hourly weather (actual + DA forecast), broadcast to 15-min.
    Full coverage 2024-01 to 2026-03. Bardejov is eastern Slovakia — not perfect
    for whole-country signal but good proxy for temperature-driven load effects."""
    print("[*] Loading Bardejov weather data...")
    ...
    if not actual_path.exists():
        print("[-] Bardejov weather not found, skipping")
        return None

    # Actuals: hourly measurements (known after the hour)
    act = pd.read_csv(actual_path, parse_dates=['time'])
    act = act.set_index('time').sort_index()
    ...
    # DA temperature forecast: known D-1, available at prediction time without lag
    if da_path.exists():
        da = pd.read_csv(da_path, parse_dates=['time'])
        da = da.set_index('time').sort_index()
        da = _dedup_index(da)
        # Use GFS seamless D+1 forecast as the DA temperature
        if 'gfs_seamless_temp_da1' in da.columns:
            weather = weather.join(da[['gfs_seamless_temp_da1']], how='left')
            weather = weather.rename(columns={'gfs_seamless_temp_da1': 'temp_forecast_da'})
        elif 'best_match_temp_da1' in da.columns:
            weather = weather.join(da[['best_match_temp_da1']], how='left')
            weather = weather.rename(columns={'best_match_temp_da1': 'temp_forecast_da'})

    # Broadcast hourly -> 15-min
    weather_15 = weather.resample('15min').ffill()
    print(f"[+] Weather: {len(weather_15)} periods, {weather_15.index.min()} to {weather_15.index.max()}")
    return weather_15
...
def build_features(reg, load, prod, xborder, solar, damas_load, da_prices,
                   market_spreads, weather, load_nowcast, imb):
    """
    ...
    # ==================================================================
    # GROUP 11: WEATHER (Bardejov, full coverage 2024-01+)
    # Actual weather: shift by 3h (hourly, known after hour ends)
    # DA forecast temperature: no shift (known D-1)
    # ==================================================================
    if 'temperature' in df.columns:
        temp_s = df['temperature'].shift(LEAD_PERIODS + 4)  # 3h lag for actuals
        features['temp'] = temp_s
        features['temp_change6h'] = temp_s - df['temperature'].shift(LEAD_PERIODS + 4 + 24)
        features['temp_rmean24h'] = temp_s.rolling(96).mean()
        features['temp_deviation'] = temp_s - temp_s.rolling(96 * 7, min_periods=96).mean()
        # DA temperature forecast for target hour (known D-1, no lag)
        if 'temp_forecast_da' in df.columns:
            features['temp_forecast_da'] = df['temp_forecast_da']
            # Forecast surprise: how wrong was yesterday's temp forecast?
            temp_surprise = df['temperature'] - df['temp_forecast_da']
            features['temp_surprise_lag'] = temp_surprise.shift(LEAD_PERIODS + 4)
        else:
            features['temp_forecast_da'] = pd.Series(np.nan, index=df.index)
            features['temp_surprise_lag'] = pd.Series(np.nan, index=df.index)
        # Wind, radiation, pressure (lagged actuals)
        if 'windspeed' in df.columns:
            features['windspeed'] = df['windspeed'].shift(LEAD_PERIODS + 4)
        else:
            features['windspeed'] = pd.Series(np.nan, index=df.index)
        if 'radiation' in df.columns:
            features['radiation'] = df['radiation'].shift(LEAD_PERIODS + 4)
        else:
            features['radiation'] = pd.Series(np.nan, index=df.index)
        if 'cloudcover' in df.columns:
            features['cloudcover'] = df['cloudcover'].shift(LEAD_PERIODS + 4)
        else:
            features['cloudcover'] = pd.Series(np.nan, index=df.index)
        if 'pressure' in df.columns:
            pressure_s = df['pressure'].shift(LEAD_PERIODS + 4)
            features['pressure_change6h'] = pressure_s - df['pressure'].shift(LEAD_PERIODS + 4 + 24)
        else:
...
def evaluate(pred_df, models, feature_cols, df):
    ...
    group_map = {
        ...
        'Weather': lambda f: f.startswith(('temp', 'wind', 'rad', 'cloud', 'pressure')),
...
def plot_results(pred_df, imp, feature_cols):
    ...
    colors_map = {
        ...
        'imb_': '#c0392b', 'spread': '#c0392b',
        'temp': '#2980b9', 'wind': '#2980b9', 'rad': '#2980b9',
        'cloud': '#2980b9', 'pressure': '#2980b9',
